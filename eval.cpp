...
const double   MIN_OVERLAP[3][NUM_CLASS] = {{0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7},
                                            {0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7},
                                            {0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7}};
...
bool eval(string result_dir){

  // set some global parameters
  initGlobals();

  // ground truth and result directories
  string gt_dir         = "kitti_data/training/label_2";
  ...
  vector<int> indices = getEvalIndices(result_dir + "/data");
  std::cout << indices.size() << " results found." << std::endl;

  int N_TESTIMAGES = indices.size();
